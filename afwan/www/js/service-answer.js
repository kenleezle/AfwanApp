...
$(document).ready(function() {
    ...
    // console.log(window.localStorage.getItem("respondID"));
    $('.btn-default').click(function(e) {
        var respondID = window.localStorage.getItem("respondID")
        console.log(destLong)
        var datastr="";
        if(!destLong)
        {
            datastr= '{"content": "' + $('#response').val() + '","userID": 3,"requestID": ' + respondID + '}'
        }
        else
        {
        ...
        console.log(datastr)
        $.ajax({
            url: 'http://ec2-54-149-21-125.us-west-2.compute.amazonaws.com/res/',
            type: "POST",
            data:datastr,
            success: function() {
